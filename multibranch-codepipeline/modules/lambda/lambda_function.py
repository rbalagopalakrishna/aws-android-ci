...
def lambda_handler(event, context):
    ...
    if event_type == 'referenceCreated':
        ...
        print("Source Branch Name :: %s" % branch_ref)
        ...
        time.sleep(10)
        getpipeline = get_status(new_pipeline_response['pipeline']['name'])
        source_execution_status = getpipeline['stageStates'][0]['latestExecution']['status']
        build_execution_status = getpipeline['stageStates'][1]['latestExecution']['status']
        if source_execution_status == 'Succeeded':
            print('Stage Source Successfully Passed')
            count = 0
            while count < 1000:
                if build_execution_status == 'Succeeded':
                    print('Build Successfully Passed')
                    break
                elif build_execution_status == 'Failed':
                    pipeline_build_status = get_status(new_pipeline_response['pipeline']['name'])
                    print(pipeline_build_status['stageStates'][1]['actionStates'][0]['latestExecution']['errorDetails']['message'])
                    break
                else:
                    pipeline_build_status = get_status(new_pipeline_response['pipeline']['name'])
                    build_status = pipeline_build_status['stageStates'][1]['latestExecution']['status']
    ...
    elif event_type == 'pullRequestSourceBranchUpdated' or event_type == 'pullRequestCreated':
                    ...
                    if stage['stageName'] == 'Source':
                        ...
                        while count < 500:
                            ...
                            if build_execution_status == 'Succeeded':
                                ...
                                if destination_branch == 'master':
                                    print('Keeping Master Branch Pipelines for reference.')
                                else:
